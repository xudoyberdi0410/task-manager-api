...
class TestTaskServiceSecurity:
    ...
    def test_update_task_status_security(self, db_session, test_user, another_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            task_service.update_task_status(
                task.task_id, StatusEnum.done, another_user.user_id
            )
    ...
    def test_get_task_statistics_isolation(self, db_session, test_user, another_user):
        ...
        task_service.create_task(
            TaskCreate(title="Todo Task", status=StatusEnum.todo), test_user.user_id
        )
        task_service.create_task(
        ...
        task_service.create_task(
            TaskCreate(title="Done Task", status=StatusEnum.done), test_user.user_id
        )
