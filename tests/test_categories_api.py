...
def auth_headers(client, test_user_for_api):
    ...
    response = client.post("/token", data={
        "username": test_user_for_api.email,
        "password": "password123"
    })
...
def test_get_category_by_id_api(client, auth_headers, db_session, test_user_for_api):
    ...
    # Создаем категорию
    category_repo = CategoryRepository(db_session)
    category = category_repo.create_category("Тестовая категория", test_user_for_api.user_id)
    
    response = client.get(f"/api/categories/{category.category_id}", headers=auth_headers)
...
def test_update_category_api(client, auth_headers, db_session, test_user_for_api):
    ...
    # Создаем категорию
    category_repo = CategoryRepository(db_session)
    category = category_repo.create_category("Старое название", test_user_for_api.user_id)
...
def test_patch_category_api(client, auth_headers, db_session, test_user_for_api):
    ...
    # Создаем категорию
    category_repo = CategoryRepository(db_session)
    category = category_repo.create_category("Исходное название", test_user_for_api.user_id)
...
def test_delete_category_api(client, auth_headers, db_session, test_user_for_api):
    ...
    # Создаем категорию
    category_repo = CategoryRepository(db_session)
    category = category_repo.create_category("Удаляемая категория", test_user_for_api.user_id)
    
    response = client.delete(f"/api/categories/{category.category_id}", headers=auth_headers)
    
    assert response.status_code == 204
    
    # Проверяем, что категория действительно удалена
    get_response = client.get(f"/api/categories/{category.category_id}", headers=auth_headers)
...
def test_get_categories_with_pagination_api(client, auth_headers, db_session, test_user_for_api):
...
def test_get_categories_count_api(client, auth_headers, db_session, test_user_for_api):
    ...
    category_repo = CategoryRepository(db_session)
    for i in range(3):
        category_repo.create_category(f"Счетная категория {i+1}", test_user_for_api.user_id)
