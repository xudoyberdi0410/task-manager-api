...
class TestTasksSecurity:
    ...
    def test_cannot_access_other_user_task(
        self, client: TestClient, test_user_data: dict, another_user_data: dict
    ):
        ...
        )  # Задача не найдена (для второго пользователя)
    ...
    def test_cannot_update_other_user_task(
        ...
        update_data = {"title": "Hacked Task", "description": "This should not work"}
        response = client.put(
            f"/api/tasks/{task_id}", json=update_data, headers=headers2
        )
        assert (
            response.status_code == 404
        )  # Задача не найдена (для второго пользователя)
    ...
    def test_cannot_update_task_status_of_other_user(
        self, client: TestClient, test_user_data: dict, another_user_data: dict
    ):
        ...
        # Первый пользователь создает задачу
        task_data = {"title": "Status Protection Task", "status": "todo"}
        response = client.post("/api/tasks/", json=task_data, headers=headers1)
        assert response.status_code == 201
        task_id = response.json()["task_id"]
        ...
        response = client.patch(
            f"/api/tasks/{task_id}/status?new_status=done", headers=headers2
        )
        assert (
            response.status_code == 404
        )  # Задача не найдена (для второго пользователя)
    ...
    def test_tasks_isolation_in_list(
        self, client: TestClient, test_user_data: dict, another_user_data: dict
    ):
    ...
    def test_bulk_operations_isolation(
        ...
        response = client.patch(
            "/api/tasks/bulk/status", json=bulk_data, headers=headers2
        )
    ...
    def test_bulk_delete_isolation(
        self, client: TestClient, test_user_data: dict, another_user_data: dict
    ):
        ...
        response = client.request(
            "DELETE", "/api/tasks/bulk", json=bulk_data, headers=headers2
        )
        assert response.status_code == 200
    ...
    def test_category_tasks_isolation(
        self, client: TestClient, test_user_data: dict, another_user_data: dict
    ):
        ...
        response1 = client.post(
            "/api/categories/", json=category_data, headers=headers1
        )
        category1_id = response1.json()["category_id"]

        response2 = client.post(
            "/api/categories/", json=category_data, headers=headers2
        )
        ...
        assert (
            response.status_code == 404
        )  # Категория не найдена (для первого пользователя)
        ...
        assert (
            response.status_code == 404
        )  # Категория не найдена (для второго пользователя)
