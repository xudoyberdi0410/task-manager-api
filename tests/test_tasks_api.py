...
class TestTasksAPI:
    ...
    def test_create_task_with_category(
        self, client: TestClient, auth_headers: dict, test_category: dict
    ):
    ...
    def test_get_tasks_with_filters(self, client: TestClient, auth_headers: dict):
        ...
        task_data = {"title": "In Progress Task", "status": "in_progress"}
        client.post("/api/tasks/", json=task_data, headers=auth_headers)
    ...
    def test_get_task_by_id(
        self, client: TestClient, auth_headers: dict, test_task: dict
    ):
    ...
    def test_update_task_status(
        ...
        response = client.patch(
            f"/api/tasks/{task_id}/status?new_status={new_status}", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_get_overdue_tasks(self, client: TestClient, auth_headers: dict):
        ...
        task_data = {"title": "Overdue Task", "due_date": past_date, "status": "todo"}
        client.post("/api/tasks/", json=task_data, headers=auth_headers)
    ...
    def test_bulk_update_status(self, client: TestClient, auth_headers: dict):
        ...
        update_data = {"task_ids": task_ids, "new_status": "done"}
        response = client.patch(
            "/api/tasks/bulk/status", json=update_data, headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_bulk_delete_tasks(self, client: TestClient, auth_headers: dict):
        ...
        response = client.request(
            "DELETE", "/api/tasks/bulk", json=delete_data, headers=auth_headers
        )
        assert response.status_code == 200
