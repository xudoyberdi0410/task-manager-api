...
class CategoryService:
    ...
    def get_category_by_id(self, category_id: int, user_id: int) -> Optional[CategoryResponse]:
    ...
    def get_categories_by_user(self, user_id: int, skip: int = 0, limit: int = 100) -> Tuple[List[CategoryResponse], int]:
    ...
    def search_categories(self, query: str, user_id: int, skip: int = 0, limit: int = 100) -> Tuple[List[CategoryResponse], int]:
        """Поиск категорий по названию"""
        categories, total = self.repository.search_categories(query, user_id, skip, limit)
        category_responses = [CategoryResponse.model_validate(cat) for cat in categories]
        return category_responses, total
    
    def create_category(self, category_data: CategoryCreate, user_id: int) -> Optional[CategoryResponse]:
    ...
    def update_category(self, category_id: int, category_data: CategoryUpdate, user_id: int) -> Optional[CategoryResponse]:
        """Обновить категорию"""
        ...
        if (category_data.title is not None and 
            self.repository.exists_by_title_except_category(category_data.title, user_id, category_id)):
            return None
