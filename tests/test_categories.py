...
def test_get_category_by_id_success(category_service, test_user):
    ...
    created_category = category_service.create_category(
        category_data, test_user.user_id
    )

    retrieved_category = category_service.get_category_by_id(
        created_category.category_id, test_user.user_id
    )
...
def test_update_category_success(category_service, test_user):
    ...
    updated_category = category_service.update_category(
        created_category.category_id, update_data, test_user.user_id
    )
...
def test_update_category_duplicate_title(category_service, test_user):
    ...
    _category1 = category_service.create_category(category1_data, test_user.user_id)
    ...
    updated_category = category_service.update_category(
        category2.category_id, update_data, test_user.user_id
    )
...
def test_update_category_not_found(category_service, test_user):
    ...
    update_data = CategoryUpdate(title="Новое название")
    updated_category = category_service.update_category(
        999, update_data, test_user.user_id
    )
...
def test_delete_category_success(category_service, test_user):
    ...
    deleted_category = category_service.get_category_by_id(
        created_category.category_id, test_user.user_id
    )
...
def test_category_exists(category_service, test_user):
    ...
    exists = category_service.category_exists(
        created_category.category_id, test_user.user_id
    )
...
def test_repository_get_by_title(category_repository, test_user):
    ...
    category = category_repository.create_category(
        "Тестовая категория", test_user.user_id
    )

    # Получаем по названию
    found_category = category_repository.get_by_title(
        "Тестовая категория", test_user.user_id
    )
...
def test_repository_exists_by_title(category_repository, test_user):
    ...
    exists = category_repository.exists_by_title(
        "Проверяемая категория", test_user.user_id
    )
    assert exists is True

    # Проверяем несуществующую категорию
    not_exists = category_repository.exists_by_title(
        "Несуществующая категория", test_user.user_id
    )
    assert not_exists is False
