...
def test_search_users(client, test_user_data):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = client.get(
        f"/api/users?search={test_user_data['email']}", headers=headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_update_user_me(client, test_user_data):
    ...
    response = client.put(
        "/api/users/me", json={"username": new_username}, headers=headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_update_user_by_id(client, test_user_data):
    ...
    response = client.put(
        f"/api/users/{user_id}", json={"username": new_username}, headers=headers
    )
    assert response.status_code == 200
    data = response.json()
    assert data["username"] == new_username
