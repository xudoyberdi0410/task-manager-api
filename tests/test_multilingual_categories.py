...
class TestMultilingualCategories:
    ...
    def test_basic_multilingual_categories(self, category_service, multilingual_test_user):
        ...
        for title in multilingual_categories:
            ...
            category = category_service.create_category(category_data, multilingual_test_user.user_id)
            
            assert category is not None, f"Не удалось создать категорию: '{title}'"
            assert category.title == title, f"Название категории изменилось: ожидалось '{title}', получено '{category.title}'"
            created_categories.append(category)
        
        assert len(created_categories) == len(multilingual_categories)
        
        # Проверяем получение категорий
        all_categories, total = category_service.get_categories_by_user(multilingual_test_user.user_id)
    ...
    def test_emoji_categories(self, category_service, multilingual_test_user):
        ...
        for title in emoji_categories:
            ...
            assert category is not None, f"Не удалось создать категорию с эмодзи: '{title}'"
            assert category.title == title, f"Эмодзи изменились: ожидалось '{title}', получено '{category.title}'"
            created_categories.append(category)
    ...
    def test_special_symbols_categories(self, category_service, multilingual_test_user):
        ...
        for title in special_categories:
            ...
            assert category is not None, f"Не удалось создать категорию со спецсимволами: '{title}'"
            assert category.title == title, f"Спецсимволы изменились: ожидалось '{title}', получено '{category.title}'"
    ...
    def test_mixed_language_categories(self, category_service, multilingual_test_user):
        ...
        for title in mixed_categories:
            ...
            assert category is not None, f"Не удалось создать смешанную категорию: '{title}'"
            assert category.title == title, f"Смешанное название изменилось: ожидалось '{title}', получено '{category.title}'"
    
    def test_unicode_high_order_characters(self, category_service, multilingual_test_user):
        ...
        for title in unicode_categories:
            ...
            assert category is not None, f"Не удалось создать Unicode категорию: '{title}'"
            assert category.title == title, f"Unicode символы изменились: ожидалось '{title}', получено '{category.title}'"
    ...
    def test_multilingual_search(self, category_service, multilingual_test_user):
        ...
        for title in test_categories:
            category_data = CategoryCreate(title=title)
            category_service.create_category(category_data, multilingual_test_user.user_id)
        ...
        for search_term, expected_count in search_tests:
            found_categories, _ = category_service.search_categories(search_term, multilingual_test_user.user_id)
            assert len(found_categories) == expected_count, \
    ...
    def test_long_multilingual_names(self, category_service, multilingual_test_user):
        ...
        for title in long_names:
            ...
            assert category is not None, f"Не удалось создать длинное название ({len(title)} символов): '{title}'"
            assert category.title == title, f"Длинное название изменилось: ожидалось '{title}', получено '{category.title}'"
    ...
    def test_multilingual_update(self, category_service, multilingual_test_user):
        ...
        assert updated_category.title == new_title, \
            f"Обновление не сработало: ожидалось '{new_title}', получено '{updated_category.title}'"
    
    def test_empty_and_whitespace_handling(self, category_service, multilingual_test_user):
        ...
        for title in invalid_titles:
            ...
            try:
                ...
                if category:
                    assert category.title.strip() != "", f"Создана категория с пустым названием: '{category.title}'"
            except Exception:
