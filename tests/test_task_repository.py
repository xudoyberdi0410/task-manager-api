...
class TestTaskRepository:
    ...
    def test_get_all_by_user_with_filters(self, task_repo, user, db_session):
        ...
        task2 = Task(title="Task 2", status=StatusEnum.in_progress, user_id=user.user_id)
    ...
    def test_get_by_category(self, task_repo, user, category, db_session):
        ...
        task1 = Task(title="Task 1", category_id=category.category_id, user_id=user.user_id)
    ...
    def test_search_tasks(self, task_repo, user, db_session):
        ...
        task1 = Task(title="Important meeting", description="Weekly team meeting", user_id=user.user_id)
        task2 = Task(title="Buy groceries", description="Milk, bread, eggs", user_id=user.user_id)
        task3 = Task(title="Code review", description="Review PR for important feature", user_id=user.user_id)
    ...
    def test_count_by_status(self, task_repo, user, db_session):
        ...
        # Создаем задачи с разными статусами
        statuses = [StatusEnum.todo, StatusEnum.todo, StatusEnum.in_progress, StatusEnum.done]
        for status in statuses:
            task = Task(title=f"Task {status.value}", status=status, user_id=user.user_id)
            db_session.add(task)
        db_session.commit()
        ...
        in_progress_count = task_repo.count_by_status(user.user_id, StatusEnum.in_progress)
